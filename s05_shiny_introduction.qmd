...
- Programmatic Data Access (contentid, EDI data) - data loading approach echoed here
...
- <u>Define</u> the key components of a Shiny application (UI, server, reactivity)
- <u>Create</u> a single-file Shiny app using modern `bslib` layouts
- <u>Apply</u> the three rules for building reactive server outputs
- <u>Use</u> `reactive()` to create reactive data objects in the server
- <u>Compare</u> single-file and two-file app structures
- <u>Identify</u> options for sharing Shiny apps with others
:::
...
::: column-margin
![](images/shiny/shiny-hex.png){width="80%" fig-align="right"}
:::
...
![Source: Intro to Shiny - EDS 430, Bren School UCSB](images/shiny/basic_shiny_app.png){.lightbox}
...
![](images/shiny/basic-widgets.png){width="80%" .lightbox}
...
![Source: Intro to Shiny - EDS 430, Bren School UCSB](images/shiny/reactivity-intro.png){width="80%" .lightbox}
...
yolo-bypass-app/
├── app.R
├── scratch.R <- for prototyping plots and data wrangling (not part of the app)
└── yolo-bypass-app.Rproj <- project file if using RStudio
```
...
1. Create a new folder for your app (e.g., `yolo-bypass-app`).
...
1. Create a new project: **File → New Project → New Directory → Shiny Application**. Name it `yolo-bypass-app`.
...
Code sections (denoted by `# section name ----`) appear as collapsible entries in Positron's **Outline** panel and RStudio's **Document Outline**, making large `app.R` files much easier to navigate.
:::
...
We'll use `contentid` to load the data reliably from the EDI data repository:
...
We'll add a `sliderInput()` inside `sidebar()`:
...
```
1. The `inputId` is how you'll refer to this input's current value in the server. Keep it short and descriptive, and remember that spelling matters!

### Step 2: Add a plot output placeholder {.unnumbered}

Output functions in the UI create **placeholders**. These are slots where server-generated content will appear. We'll add a `plotOutput()` inside a `card()`:
...
ui <- page_sidebar(
  ...
  # Secchi depth plot ----
  card(
    plotOutput(outputId = "secchi_plot")   # <1>
  )

)
```
1. The `outputId` is **quoted** in the UI. In the server, you'll refer to it as `output$secchi_plot` (unquoted, with `$`). These must match exactly.
...
![](images/shiny/shiny-yolo-app-part-1.png){width="80%" fig-align="center" .lightbox}

::: {.callout-important}
## Common mistakes

- Misspelling `inputId` as `inputID` (or `outputId` as `outputID`) -- R is case-sensitive.
- Mismatched IDs between UI and server (e.g., `"secchi_plot"` in UI but `secchi_Plot` in server)
...
![](images/shiny/shiny-yolo-app-part-2.png){width="80%" fig-align="center" .lightbox}
...
![Source: Intro to Shiny - EDS 430, Bren School UCSB](images/shiny/one-vs-two.png)
...
A **third optional file**, `global.R`, runs once before `ui.R` and `server.R`. It's the right place for packages, data loading, and any objects that both files need.

### What goes in `global.R`? {.unnumbered}
...
```
yolo-bypass-app/
├── global.R          ← packages + data (runs first, available everywhere)
├── ui.R              ← user interface definition
├── server.R          ← server logic
├── text/
│   └── about.md      ← markdown text for the About page
├── scratch.R         ← development scratch work (not part of the app)
└── yolo-bypass-app.Rproj <- project file if using RStudio
```

![Source: Intro to Shiny - EDS 430, Bren School UCSB](images/shiny/repo-structure-details.png){width="80%"}
...
![](images/shiny/shiny-yolo-app-tabbed-about.png){width="80%" .lightbox}

#### Explore page

![](images/shiny/shiny-yolo-app-tabbed-data.png){width="80%" .lightbox}

:::

### Build the navbar structure in `ui.R` {.unnumbered}
...
Put `"Plot will go here"` or similar placeholder strings inside each `nav_panel()` so you can see the structure as you build. Run the app after each addition. Catching errors early saves a lot of debugging time!
:::

### Add the Explore tab layout {.unnumbered}
...
### Add the About page with Markdown text {.unnumbered}
...
### Write the server in `server.R` {.unnumbered}
...
2. Follow the on-screen instructions to link your account, which involves running a few lines from the `rsconnect` package in your Console
3. Publish your app:
...
Click the blue **Publish** button (cloud icon) in the top-right corner of the running app window.
:::

![](images/shiny/shiny-io-account.png){width="70%"}
...
# export app to a self-contained static site
shinylive::export(appdir = "yolo-bypass-app/", destdir = "site/")
```
