...
Partition the penguins data, with 85% going into our training set (which we will resample later as 70%-15%), and 15% set aside for final testing.  We will stratify on the `sex` column (`strata = sex`), so each sex will have the same proportional representation in our training and test sets!
    
```{r}
set.seed(42)
peng_split <- initial_split(penguins_clean, prop = 0.85, strata = sex)
...
# # 0.2352941 0.3137255 0.4509804;  0.4751773 0.1843972 0.3404255
# 
# table(peng_test_df$sex) / nrow(peng_test_df); table(peng_train_df$sex) / nrow(peng_train_df)
# #    female      male;     female      male 
# # 0.4901961 0.5098039;  0.4964539 0.5035461
```
...
For classification problems, a "confusion matrix" shows True Positives and Negatives vs. False Positives and Negatives. Female (`sex) vs. female (`.pred_class`) = True Negative; male/female = False Negative; male/male = True Positive; female/male = False Positive.
...
A low threshold means that, even at very low predicted probabilities, we assign "Male" - which means we get most of the males (high True Positive Rate), but mis-classify a lot of females as males (high False Positive Rate).  A high threshold means we need a high predicted probability to classify a penguin as male - meaning we err on the side of classifying as female, i.e., a high True Negative Rate - and misclassify many males as females (high False Negative Rate).
...
Resample the data into five folds (`v = 5`) in a single line - and optionally lets us also do *repeated* rounds of cross validation (`repeats` argument) at the same time!

```{r}
peng_train_folds <- vfold_cv(peng_train_df, v = 5, repeats = 10) # <1>
```
...
Based on these performance evaluation metrics, our final model would use specification 1 with a support vector machine algorithm.

## Model Tuning

Having chosen a model from a suite of candidate models, we now want to make sure our model is performing at peak effectiveness.  Each model algorithm and engine relies on a set of _hyperparameters_, essentially configuration settings that can affect its computational and predictive performance - especially to avoid **overfitting** and **underfitting**.  Model parameters (such as model coefficients and weights) are learned from the data, but _hyperparameters_ control the learning process itself.
...
Typically, a generic model will start from a pretty good default set of hyperparameters.  But with a little work we can test different values of hyperparameters to see how model performance changes, searching for an optimal set (usually across multiple hyperparameters simultaneously).  Three common options for tuning:
...
<!-- For this workshop/tutorial, we will use the `tidymodels` package to quickly generate and test predictive models, both regression models (predicting a continuous output based on various inputs) and classification models (predicting a categorical output based on various inputs). -->

<!-- This borrows heavily from the Posit `tidymodels` tutorial that can be found here - visit these pages for more in-depth exploration of the `tidymodels` package: -->
