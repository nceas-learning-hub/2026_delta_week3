...
::::{.column width="30%"}
![](images/r_machine_learning_tidymodels/all_ml.jpg){.lightbox width=95%}
::::
::::{.column width="70%"}
<br><br>
...
![](images/r_machine_learning_tidymodels/grid_search.png){fig-width="70%" fig-align="center" .lightbox}
...
![](images/r_machine_learning_tidymodels/randomized_search.png){fig-width="70%" fig-align="center" .lightbox}
...
![](images/r_machine_learning_tidymodels/bayesian_optimization.png){fig-width="70%" fig-align="center" .lightbox}
...
Often we want to know which variables were most influential or important to the predictive power of our Machine Learning model.  For a simple linear regression, the magnitude, sign, and p value (or t score) of the coefficients gives us a lot of information about the influence of the variables.  For more complex ML models, this can be trickier.

Fortunately, the `vip` package ("Variable Importance") combines (nearly) seamlessly with `tidymodels`.  All we need to do is extract our fit information, using `tune::extract_fit_parsnip` (since our fit is performed using `parsnip` functions), and then hand this off to the `vip::vip()` function.

**UN**fortunately, this doesn't work well on `svm_linear` models - not well supported (yet?).  And for certain models, you may need to set parameters in the definition of the model.  (It can be a bit finicky and model dependent!  be patient and Google well!)

To demonstrate, let's recreate a final fit using our Binomial Logistic Regression model instead:
...
![](images/r_machine_learning_tidymodels/knz_bison.png){fig-width='70%' fig-align='center' .lightbox}
...
![](images/r_machine_learning_tidymodels/bison_plot.png){fig-width='70%' fig-align='center' .lightbox}
